...
router.get('/attendances', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM attendances ORDER BY date_created DESC; SELECT count(*) as count FROM attendances',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/attendances', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO attendances (student_id) VALUES ('${student_id}')`,
               function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/attendances/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, student_id FROM attendances WHERE id = '${req.params.id}'`,
...
router.put('/attendances/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        sanitizeBody('student_id').trim().escape();
        const student_id = req.body.student_id;
        new sql.Request().query(`UPDATE attendances SET student_id = '${student_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/attendances/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM attendances WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/courses', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM courses ORDER BY date_created DESC; SELECT count(*) as count FROM courses',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/courses', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO courses (name, description, section_id, department_id, room_id) VALUES ('${name}','${description}','${section_id}','${department_id}','${room_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/courses/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, name, description, section_id, department_id, room_id FROM courses WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/courses/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const department_id = req.body.department_id;
        const room_id = req.body.room_id;
        new sql.Request().query(`UPDATE courses SET name = '${name}', description = '${description}', section_id = '${section_id}', department_id = '${department_id}', room_id = '${room_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/courses/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM courses WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/departments', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM departments ORDER BY date_created DESC; SELECT count(*) as count FROM departments',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/departments', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO departments (name, description, teacher_id) VALUES ('${name}','${description}','${teacher_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/departments/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, name, description, teacher_id FROM departments WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/departments/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const description = req.body.description;
        const teacher_id = req.body.teacher_id;
        new sql.Request().query(`UPDATE departments SET name = '${name}', description = '${description}', teacher_id = '${teacher_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/departments/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM departments WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/rooms', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM rooms ORDER BY date_created DESC; SELECT count(*) as count FROM rooms',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/rooms', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO rooms (number) VALUES ('${number}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/rooms/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, number FROM rooms WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/rooms/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        sanitizeBody('number').trim().escape();
        const number = req.body.number;
        new sql.Request().query(`UPDATE rooms SET number = '${number}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/rooms/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM rooms WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/sections', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM sections ORDER BY date_created DESC; SELECT count(*) as count FROM sections',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/sections', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO sections (name, start_time, end_time) VALUES ('${name}','${start}','${end}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/sections/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, name, start_time, end_time FROM sections WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/sections/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const start = req.body.start;
        const end = req.body.end;
        new sql.Request().query(`UPDATE sections SET name = '${name}', start_time = '${start}', end_time = '${end}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/sections/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM sections WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/students', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM students ORDER BY date_created DESC; SELECT count(*) as count FROM students',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/students', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO students (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/students/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM students WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/students/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        new sql.Request().query(`UPDATE students SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
...
router.delete('/students/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM students WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/studentscourses', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM studentscourses ORDER BY date_created DESC; SELECT count(*) as count FROM studentscourses',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/studentscourses', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            new sql.Request().query(`INSERT INTO studentscourses (grade, student_id, course_id) VALUES ('${grade}', '${student_id}','${course_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/studentscourses/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, grade, student_id, course_id FROM studentscourses WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/studentscourses/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const student_id = req.body.student_id;
        const course_id = req.body.course_id;
        new sql.Request().query(`UPDATE studentscourses SET grade = '${grade}', student_id = '${student_id}', course_id = '${course_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/studentscourses/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM studentscourses WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/teachers', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM teachers ORDER BY date_created DESC; SELECT count(*) as count FROM teachers',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/teachers', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const gender_id = req.body.gender_id;
            const address_id = req.body.address_id;
            new sql.Request().query(`INSERT INTO teachers (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/teachers/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM teachers WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/teachers/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        new sql.Request().query(`UPDATE teachers SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
            email = '${email}', phone_number = '${phone_number}', gender_id = '${gender_id}', address_id = '${address_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
...
router.delete('/teachers/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM teachers WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/teacherscourses', function(req, res, next) {
    if (req.isAuthenticated()) {
        new sql.Request().query('SELECT * FROM teacherscourses ORDER BY date_created DESC; SELECT count(*) as count FROM teacherscourses',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/teacherscourses', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const teacher_id = req.body.teacher_id;
            const course_id = req.body.course_id;
            new sql.Request().query(`INSERT INTO teacherscourses (teacher_id, course_id) VALUES ('${teacher_id}','${course_id}')`,
                function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.get('/teacherscourses/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        new sql.Request().query(`SELECT id, teacher_id, course_id FROM teacherscourses WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/teacherscourses/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const teacher_id = req.body.teacher_id;
        const course_id = req.body.course_id;
        new sql.Request().query(`UPDATE teacherscourses SET teacher_id = '${teacher_id}', course_id = '${course_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/teacherscourses/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        new sql.Request().query(`DELETE FROM teacherscourses WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
